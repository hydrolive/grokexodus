...
- Visual caves: remesh **only edited chunks** when the brush is under the stamp (true cave). Surface bowls are the **clipmap**: one **80 m / 2 m** walk disk (no second 2 m lid). Verts drop to a preview-sized bowl, or to the excavated column floor (skip the stamp crust — that was the leftover grass sheet). Walk disk is not stamp-reset for 2.5 s after a click. Tool ray skips a lid over auth air so the ball sits in the hole. Clipmap **never deletes quads**. Banks do not cast VSM shadows. Brush sits 0.35 R into the hit.
...
| Band | Default |
|---|---|
| Collision / LOD0 | full stream (260 m) until transvoxel skirts |
| Mesh stream | 260 m |
...
LOD (0.8): **0 underfoot**, **1** past near field, **2** past stream. Voxel transvoxel skirts only on LOD≥1 (LOD0 cave skirts were floating black faces). Collision is stamp-snap outside edits. Clipmap HLOD: **one** walk disk 2 m / 80 m; mid 8 m from 70 m (sunk 2.4 m); far 32 / 96 m with rim skirts. Clipmap material is grass + shader slope blend (no hard rock/dirt IDs). `CreateMeshSection` hitch is not HLOD’s job.
