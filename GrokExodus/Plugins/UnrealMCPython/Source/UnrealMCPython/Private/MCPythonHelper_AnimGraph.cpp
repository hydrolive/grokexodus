...
/** Spawn a BlendSpace player and wire X/Y coordinate pins to self-member float variables. */
static UAnimGraphNode_BlendSpacePlayer* SpawnBlendSpacePlayer(UEdGraph* Graph, UBlendSpace* BS,
    const FString& XVar, const FString& YVar, int32 PosX, int32 PosY, UEdGraphPin* PoseSinkInputPin)
{
    if (!Graph || !BS) return nullptr;

    FGraphNodeCreator<UAnimGraphNode_BlendSpacePlayer> Creator(*Graph);
    UAnimGraphNode_BlendSpacePlayer* Node = Creator.CreateNode(false);
    Node->Node.SetBlendSpace(BS);
    Node->Node.SetLoop(true);
    Node->NodePosX = PosX;
    Node->NodePosY = PosY;
    Creator.Finalize();

    auto WireVarToPin = [&](const FString& VarName, const TCHAR* PinName, int32 OffsetY)
    {
        if (VarName.IsEmpty()) return;
        UEdGraphPin* CoordPin = FindPinByName(Node, PinName, EGPD_Input);
        if (!CoordPin)
        {
            // UE5 may name pins "X" / "Y" or expose as BlendSpace coordinates
            for (UEdGraphPin* P : Node->Pins)
            {
                if (P && P->Direction == EGPD_Input && P->PinName.ToString().Equals(PinName, ESearchCase::IgnoreCase))
                {
                    CoordPin = P;
                    break;
                }
        ...
        GetNode->NodePosX = PosX - 350;
        GetNode->NodePosY = PosY + OffsetY;
        GetCreator.Finalize();
        if (UEdGraphPin* VarOut = FirstVisiblePin(GetNode, EGPD_Output))
            VarOut->MakeLinkTo(CoordPin);
    };

    WireVarToPin(XVar, TEXT("X"), -40);
    WireVarToPin(YVar, TEXT("Y"), 40);
...
static FString BuildStateMachineFromSpec(UAnimBlueprint* AnimBP, const TSharedPtr<FJsonObject>& Spec)
{
    ...
    // Resolve + validate every state's anim / blendspace up front (fail before mutating the graph).
    struct FStateDef
    {
        FString Name;
    ...
    for (const TSharedPtr<FJsonValue>& SV : *StatesJson)
    {
        ...
        SO->TryGetStringField(TEXT("blendspace"), BSPath);
        FStateDef Def;
        Def.Name = Name;
        SO->TryGetStringField(TEXT("x_var"), Def.XVar);
        SO->TryGetStringField(TEXT("y_var"), Def.YVar);
        if (Def.XVar.IsEmpty()) Def.XVar = TEXT("WalkRight");
        if (Def.YVar.IsEmpty()) Def.YVar = TEXT("WalkForward");
        if (!BSPath.IsEmpty())
        {
            Def.BlendSpace = Cast<UBlendSpace>(StaticLoadObject(UBlendSpace::StaticClass(), nullptr, *BSPath));
            if (!Def.BlendSpace)
                return MakeJsonError(FString::Printf(TEXT("State '%s': BlendSpace not found: %s"), *Name, *BSPath));
        }
        if (!AnimPath.IsEmpty())
        {
            Def.Seq = Cast<UAnimSequence>(StaticLoadObject(UAnimSequence::StaticClass(), nullptr, *AnimPath));
            if (!Def.Seq)
                return MakeJsonError(FString::Printf(TEXT("State '%s': AnimSequence not found: %s"), *Name, *AnimPath));
        }
        if (!Def.Seq && !Def.BlendSpace)
            return MakeJsonError(FString::Printf(TEXT("State '%s': needs 'anim' or 'blendspace'."), *Name));
        StateDefs.Add(Def);
    }
    ...
    // States — sequence player and/or blendspace player wired to the state result.
    ...
    for (const FStateDef& SD : StateDefs)
    {
        ...
        if (UAnimGraphNode_StateResult* SR = State->GetResultNodeInsideState())
        {
            ...
            if (SD.BlendSpace)
            {
                SpawnBlendSpacePlayer(State->GetBoundGraph(), SD.BlendSpace, SD.XVar, SD.YVar, -400, 0, SRIn);
            }
            else if (SD.Seq)
            {
