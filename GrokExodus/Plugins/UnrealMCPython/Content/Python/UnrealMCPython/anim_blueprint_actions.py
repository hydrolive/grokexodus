...
def ue_build_anim_state_machine(asset_path: str = None, spec: dict = None) -> str:
    """Builds an AnimGraph state machine from a spec:
    states[{name, anim?, blendspace?, x_var?, y_var?}], entry?, transitions[{from,to,var?,op?,value?}].
    blendspace states use a BlendSpace Player (coords default WalkRight / WalkForward)."""
    ...
    try:
        ...
        # Validate referenced anims / blendspaces up front.
        for s in states:
            anim = s.get("anim")
            bs = s.get("blendspace")
            ...
            if bs and not unreal.EditorAssetLibrary.does_asset_exist(bs):
                return json.dumps({"success": False,
                                   "message": f"State '{s.get('name')}': BlendSpace not found: {bs}"})
            if not anim and not bs:
                return json.dumps({"success": False,
                                   "message": f"State '{s.get('name')}': needs 'anim' or 'blendspace'."})
        # Do NOT auto-create float members for vars that already exist on the C++ parent
        # (e.g. InAirValue / WalkRight). BlueprintEditorLibrary.add_member_variable renames
        # them to InAirValue_0 and breaks transition Get nodes.
