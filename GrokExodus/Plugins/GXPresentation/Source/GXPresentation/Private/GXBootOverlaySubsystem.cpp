			...
			if (UWorld* World = GetWorld())
			{
				if (UGXSkySubsystem* Sky = World->GetSubsystem<UGXSkySubsystem>())
				{
					if (APlayerController* PC = World->GetFirstPlayerController())
					{
						...
						const float PlanetRcm = static_cast<float>(Sky->GetEphemeris().PlanetRadius * 100.0);
						...
						const float Night = FMath::Clamp((0.06f - SunUp) / 0.22f, 0.0f, 1.0f);
						...
						if (Night > 0.01f && bHaveProj)
						{
							...
							for (int32 I = 0; I < N; ++I)
							{
								...
								if (Dir.IsNearlyZero() || FVector::DotProduct(CamFwd, Dir) < 0.12f)
								{
									continue;
								}
								if (HitsPlanet(Dir))
								...
								const FVector StarWorld = CamLoc + Dir * 8.0e7f;
								const FVector4 H = ViewProj.TransformFVector4(FVector4(StarWorld, 1.0f));
								if (H.W <= 0.0f)
								{
									continue;
								}
								...
								Dot.UV = FVector2f(X * 0.5f + 0.5f, 0.5f - Y * 0.5f);
								Dot.SizePx = FMath::Clamp(4.6f - Mag * 0.55f, 1.4f, 5.2f);
								Dot.Alpha = Night * FMath::Clamp(1.05f - Mag * 0.16f, 0.22f, 1.0f);
								Dots.Add(Dot);
							}
